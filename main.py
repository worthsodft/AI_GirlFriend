...
def get_response_from_ai_gf(human_input):
    ...
    #llm = OpenAI(temperature=0.1), tongyi(temperature=0.1)
...
def translate(source,target,message):
    ...
    try:
        ...
        # 如有需要，请打印 error
        error_message = str(error)  # 将异常对象转换为字符串获取错误描述
...
def get_voices():
    ...
    if response.status_code == 200:
        ...
        print(voices)
        return jsonify(voices)  # 将音色列表以JSON格式返回
    else:
...
def send_message():
    ...
    get_voice_message(ai_output_zh,voice_id)
